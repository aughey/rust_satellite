...
use clap::Parser;
use elgato_streamdeck::{images::ImageRect, info::Kind};
use gateway::{satellite::satellite_read_command, ButtonState, Cli, Result};
    ...
    W: AsyncWrite + Unpin + Send + 'static,
{
    ...
    let kind = match config.kind.as_str() {
        "mk2" => Kind::Mk2,
        _ => {
            anyhow::bail!("Unknown kind: {}", config.kind);
        }
    };
    ...
    let res =
        tokio::try_join!(satellite_to_companion, companion_to_satellite);
...
fn companion_to_satellite<R, W>(companion_read_stream: R, satellite_write_stream: W, kind: Kind, image_format: elgato_streamdeck::info::ImageFormat) -> Result<()> where R: AsyncRead + Unpin + Send + 'static, W: AsyncWrite + Unpin {
Ok(())
}

async fn satellite_to_companion<R, W>(satellite_read_stream: R, companion_write_stream: W) -> Result<()> where R: AsyncRead + Unpin, W: AsyncWrite + Unpin {
    Ok(())
}
